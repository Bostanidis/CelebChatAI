...
// Chat helpers
export const saveChat = async (userId, characterId, messages) => {
  if (!userId || !characterId || !Array.isArray(messages)) {
    console.warn('Invalid parameters for saveChat:', { userId, characterId, messagesProvided: !!messages });
    return null;
  }

  try {
    // Upsert the chat messages using the existing unique constraint
    const { data, error } = await supabase
      .from('chats')
      .upsert({
        user_id: userId,
